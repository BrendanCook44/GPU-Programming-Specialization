...
int getUserInput()
{
    ...
    while (true)
    {
        std::cout << "Enter an input between 0 and 255: " << std::endl;
        std::cin >> userInput;
...
__host__ float * runStreamsFullAsync(float *host_mem, int num_elements)
{
    ...
    std::array<cudaStream_t, 6> cudaStreams;
    ...
    copyFromHostToDeviceAsync(host_mem, dev_mem, num_elements, cudaStreams[0]);
    ...
    std::cout << "Enter seed for kernel A1 (0-255): " << std::endl;
    int s1 = getUserInput();

    std::cout << "Enter seed for kernel A2 (0-255): " << std::endl;
    int s2 = getUserInput();
    ...
    kernelA1<<<blocksPerGrid, threadsPerBlock, 0, cudaStreams[1]>>>(dev_mem, num_elements, x1);
    kernelB1<<<blocksPerGrid, threadsPerBlock, 0, cudaStreams[2]>>>(dev_mem, num_elements);
    kernelA2<<<blocksPerGrid, threadsPerBlock, 0, cudaStreams[3]>>>(dev_mem, num_elements, x2);
    kernelB2<<<blocksPerGrid, threadsPerBlock, 0, cudaStreams[4]>>>(dev_mem, num_elements);

    // Copy device memory back to host asynchronously
    copyFromDeviceToHostAsync(dev_mem, host_mem, num_elements, cudaStreams[5]);
...
__host__ float * runStreamsBlockingKernel2StreamsNaive(float *host_mem, int num_elements)
{
    ...
    // Prepare all streams such that all kernels and memory copies execute asynchronously
    std::array<cudaStream_t, 4> cudaStreams;
    ...
    auto [threadsPerBlock, blocksPerGrid] = determineThreadBlockDimensions(num_elements);
    kernelA1<<<blocksPerGrid, threadsPerBlock, 0, cudaStreams[1]>>>(dev_mem, num_elements, x1);
    kernelB1<<<blocksPerGrid, threadsPerBlock, 0, cudaStreams[1]>>>(dev_mem, num_elements);
    kernelA2<<<blocksPerGrid, threadsPerBlock, 0, cudaStreams[2]>>>(dev_mem, num_elements, x2);
    kernelB2<<<blocksPerGrid, threadsPerBlock, 0, cudaStreams[2]>>>(dev_mem, num_elements);

    // Copy device memory back to host asynchronously
    copyFromDeviceToHostAsync(dev_mem, host_mem, num_elements, cudaStreams[3]);
    deallocateDevMemory(dev_mem);
...
__host__ float * runStreamsBlockingKernel2StreamsOptimal(float *host_mem, int num_elements)
{
    ...
    kernelA1<<<blocksPerGrid, threadsPerBlock, 0, cudaStreams[1]>>>(dev_mem, num_elements, x1);
    kernelB1<<<blocksPerGrid, threadsPerBlock, 0, cudaStreams[2]>>>(dev_mem, num_elements);
    kernelA2<<<blocksPerGrid, threadsPerBlock, 0, cudaStreams[1]>>>(dev_mem, num_elements, x2);
    kernelB2<<<blocksPerGrid, threadsPerBlock, 0, cudaStreams[2]>>>(dev_mem, num_elements);
