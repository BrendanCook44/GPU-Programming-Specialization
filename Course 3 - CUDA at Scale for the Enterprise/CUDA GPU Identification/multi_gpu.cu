...
int main() {
  ...
  int maxMultiProcessorCount = 0; // Keep track of multi processors
  ...
  for (int i = 0; i < nDevices; i++) {
    ...
    if (prop.multiProcessorCount > maxMultiProcessorCount)
    {
      maxMultiProcessorCount = prop.multiProcessorCount;
