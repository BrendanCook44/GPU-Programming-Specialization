...
__host__ cufftComplex *generateCuFFTComplexPointerFromHostComplex(int mem_size, Complex *hostComplex, int SIZE)
{
    cufftComplex *d_complex;
    // Allocate device memory
    cudaMalloc((void**)&d_complex, mem_size);
    // Copy from host to device
    cudaMemcpy(d_complex, hostComplex, mem_size, cudaMemcpyHostToDevice);
...
int main(int argc, char** argv)
{
    ...
    cufftComplex *d_a = generateCuFFTComplexPointerFromHostComplex(mem_size, a, SIZE);
    cufftComplex *d_b = generateCuFFTComplexPointerFromHostComplex(mem_size, b, SIZE);
    cufftComplex *d_c = generateCuFFTComplexPointerFromHostComplex(mem_size, c, SIZE);
