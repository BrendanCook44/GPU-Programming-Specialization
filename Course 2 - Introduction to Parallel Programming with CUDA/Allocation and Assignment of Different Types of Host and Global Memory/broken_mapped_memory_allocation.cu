...
__host__ std::tuple<float *, float *> allocateRandomHostMemory(int numElements)
{
    ...
    cudaError_t err = cudaHostAlloc((void**)&h_a, size, cudaHostAllocMapped);

    if (err != cudaSuccess)
    {
        fprintf(stderr, "Failed to allocate mapped host memory for h_a (error code %s)!\n", cudaGetErrorString(err));
        exit(EXIT_FAILURE);
    }

    float *h_b;
    // FILL IN HOST AND DEVICE MEMORY ALLOCATION CODE
    err = cudaHostAlloc((void**)&h_b, size, cudaHostAllocMapped);

    if (err != cudaSuccess)
    {
        fprintf(stderr, "Failed to allocate mapped host memory for h_b (error code %s)!\n", cudaGetErrorString(err));
        exit(EXIT_FAILURE);
    }
...
__host__ std::tuple<float *, float *, int>readCsv(std::string filename)
{
    ...
    cudaError_t err = cudaHostAlloc((float**)&h_a, size, cudaHostAllocMapped);
    ...
    float *h_b;
    // FILL IN HOST AND DEVICE MEMORY ALLOCATION CODE
    err = cudaHostAlloc((float**)&h_b, size, cudaHostAllocMapped);
...
int main(int argc, char *argv[])
{
    ...
    float *h_c;
    cudaMallocManaged((void **)&h_c, numElements*sizeof(float));
