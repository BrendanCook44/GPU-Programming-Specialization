...
__host__ std::tuple<float *, float *> allocateRandomHostMemory(int numElements)
{
    ...
    cudaError_t err = cudaMallocHost((float**)&h_a, size);
    if (err != cudaSuccess)
    {
        fprintf(stderr, "Failed to allocate pinned host vector h_a (error code %s)!\n", cudaGetErrorString(err));
        exit(EXIT_FAILURE);
    }

    float *h_b;
    // FILL IN HOST AND DEVICE MEMORY ALLOCATION CODE
    err = cudaMallocHost((float**)&h_b, size);
    if (err != cudaSuccess)
    {
        fprintf(stderr, "Failed to allocate pinned host vector h_b (error code %s)!\n", cudaGetErrorString(err));
        exit(EXIT_FAILURE);
    }
...
__host__ std::tuple<float *, float *, int>readCsv(std::string filename)
{
    ...
    if (err != cudaSuccess)
    {
        fprintf(stderr, "Failed to allocate pinned host vector d_a (error code %s)!\n", cudaGetErrorString(err));
        exit(EXIT_FAILURE);
    }
...
__host__ void executeKernel(float *d_a, float *d_b, float *h_c, int numElements, int threadsPerBlock)
{
    ...
    // Synchronize to ensure kernel completes before host accesses h_c
    err = cudaDeviceSynchronize();
    if (err != cudaSuccess)
    {
        fprintf(stderr, "Failed to synchronize after kernel (error code %s)!\n", cudaGetErrorString(err));
        exit(EXIT_FAILURE);
    }
}
