...
__host__ std::tuple<int *, int *> allocateRandomHostMemory(int numElements)
{
    ...
    cudaError_t err = cudaMallocHost(&h_b, size);

    if (err != cudaSuccess)
    {
        fprintf(stderr, "Failed to allocate pinned host memory h_b (error code %s)!\n", cudaGetErrorString(err));
        exit(EXIT_FAILURE);
    }
...
__host__ void executeKernel(int *d_a, int *d_b, int *h_c, int numElements, int threadsPerBlock, std::string mathematicalOperation)
{
    ...
    if (err != cudaSuccess)
    {
        fprintf(stderr, "Failed to copy vector d_c from device to host (error code %s)!\n", cudaGetErrorString(err));
        exit(EXIT_FAILURE);
    }
