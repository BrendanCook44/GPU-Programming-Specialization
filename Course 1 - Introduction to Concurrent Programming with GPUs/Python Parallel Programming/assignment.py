...
class Assignment(Core):
    ...
    USERNAME = "Brendan"      # TODO: Replace with your actual username
    active_threads = []
    ...
    def run(self):
        ...
        if self.test_username_equality(self.USERNAME):
            ...
            shared_variable = {'current_ticket': 0, 'lock': Lock()}

            # TODO: Create threads, assign ticket numbers starting from 0, and start them
            for i in range(self.num_threads):
                thread = Thread(target=execute_ticketing_system_participation, args=(i, self.part_id, shared_variable))
                thread.start()
